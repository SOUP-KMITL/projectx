...
attackGraph.base = {
    ...
    find: function (key, cb) {
        var base = this;
        request.get('http://localhost:9292/sessions/' + attackGraph.sessionId + this._basePath + '/' + key, function (err, res, body) {
            if (!err && res.statusCode === 200) {
                cb(null, base.new(JSON.parse(body)));
            } else {
                cb(new Error('Something went wrong: find()'));
            }
        });
    },
    all: function () {
                ...
                get: function () {
                    ...
                    if (this._has_nodes === undefined) {
                        ...
                        this._has_nodes.create = function (o, cb) {
                            var newNode = o;
                            request.post({ url: 'http://localhost:9292/sessions/' + attackGraph.sessionId + node._baseSingularPath() + base._basePath,
                                form: { properties: o }}, function (err, res, body) {
                                    if (!err && res.statusCode === 200) {
                                        ...
                                        cb && cb(new Error('Something went wrong: creating new association'));
                            });
                        };

                        // find()
                        this._has_nodes.find = function (key, cb) {
                            request.get('http://localhost:9292/sessions/' + attackGraph.sessionId + node._baseSingularPath() + base._basePath + '/' + key,
                                    function (err, res, body) {
                                if (!err && res.statusCode === 200) {
                                    var found = base.new(JSON.parse(body));
                                    found._parent = node;
                                    cb(null, found);
                                } else {
                                    cb(new Error('Something went wrong: assoc find()'));
                                }
                            });
                        };
                    }
